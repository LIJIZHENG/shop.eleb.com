    ...
    <div class="container">
        ...
        <form action="{{route('menu.store')}}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="description" class="form-control" id="exampleInputPassword1" placeholder="菜品描述" value="{{old('description')}}">
            </div>
            {{--<div class="form-group">--}}
            ...
            <div class="form-group">
                ...
                <input type="text" name="tips" class="form-control" id="exampleInputPassword1" placeholder="菜品提示" value="{{old('tips')}}">
            </div>
            {{--<div class="form-group">--}}
            ...
            <div class="form-group">
                ...
                <select name="menuclass_id" class="form-control">
                    @foreach($rows as $row)
                    <option value="{{$row->id}}" {{old('menuclass_id')==$row->id?'selected="selected"':''}}>{{$row->name}}</option>
                    @endforeach
                </select>
            </div>
            {{csrf_field()}}
